...
var GeminiChatView = class extends import_obsidian.ItemView {
  ...
  inputField;
  chat;
  // ChatSession from @google/generative-ai
  ...
  async handleSendMessage() {
    ...
    try {
      ...
      while (iterations < MAX_ITERATIONS && response.candidates[0].content.parts.some((part) => !!part.functionCall)) {
        iterations++;
        const toolResults = [];
      ...
      if (responseText) {
        ...
        const contentEl = msgEl.querySelector(".gemini-message-content");
  ...
  appendMessage(sender, text) {
    ...
    msgEl.createEl("div", { text, cls: "gemini-message-content" });
...
var GeminiAgentPlugin = class extends import_obsidian2.Plugin {
  settings;
  genAI = null;
  availableModels = ["gemini-2.5-flash", "gemini-1.5-flash", "gemini-3-pro"];
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        await this.refreshModels();
        new import_obsidian2.Notice("Gemini models refreshed");
      }
    });
    this.addCommand({
  ...
  async refreshModels() {
    ...
    try {
      ...
      if (data.models) {
        this.availableModels = data.models.map((m) => m.name.replace("models/", "")).filter((name) => name.includes("gemini"));
      }
    } catch (error) {
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        if (error.message?.includes("429") || error.message?.includes("quota")) {
          new import_obsidian2.Notice(`Rate limit hit for ${modelName}, trying fallback...`);
          continue;
        }
        throw error;
      }
  ...
  async summarizeNote(file) {
    if (!this.genAI || !this.settings) {
      new import_obsidian2.Notice("Gemini API Key not configured");
      return;
    }
    new import_obsidian2.Notice(`Summarizing ${file.basename}...`);
    try {
...
${content}`;
  ...
  async saveSettings() {
    ...
    new import_obsidian2.Notice("Gemini settings saved");
  }
};
var GeminiAgentSettingTab = class extends import_obsidian2.PluginSettingTab {
  plugin;
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Google Gemini API Key").setDesc("Enter your Google Gemini API Key. It will be stored securely in your system keychain.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Auto-accept tools").setDesc("If enabled, the agent can execute Obsidian tools automatically. If disabled, you might need to confirm actions (not yet implemented in UI).").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Excluded paths").setDesc('Comma-separated list of paths or folders to exclude from the AI agent (e.g. "Private/, SecretNote.md").').addTextArea(
