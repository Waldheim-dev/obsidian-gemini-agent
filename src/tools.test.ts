...
import { getObsidianTools, ObsidianApp, CanvasNode } from './tools';
...
describe('Obsidian Tools', () => {
	...
	describe('Exclusion List', () => {
		it('should block access to excluded paths', async () => {
			...
			expect(await tools.add_node_to_canvas({ path: 'Private/MyNote.canvas', node: {} as CanvasNode })).toContain('Error: access to Private/MyNote.canvas is excluded');
		});
	});
	...
	describe('create_note', () => {
		it('should create a note with content and tags', async () => {
			...
			(mockApp.vault.create as any).mockResolvedValue(mockFile);
	...
	describe('list_files', () => {
		it('should list files in a folder', async () => {
			...
			const mockSub = new TFolder();
			mockSub.path = 'sub/';
			mockSub.children = [];
			const mockFolder = new TFolder();
			mockFolder.children = [mockFile1, mockSub];
		...
		it('should list files recursively', async () => {
			...
			mockFile2.path = 'sub/file2.md';
			mockFile2.extension = 'md';
			const mockSub = new TFolder();
			mockSub.path = 'sub/';
			mockSub.children = [mockFile2];
			const mockFile1 = new TFile();
			mockFile1.path = 'file1.md';
			mockFile1.extension = 'md';
			const mockFolder = new TFolder();
	...
	describe('Canvas Tools', () => {
		...
		it('should add a node to canvas', async () => {
			...
			const result = await tools.add_node_to_canvas({ path: 'test.canvas', node: { id: '1' } as CanvasNode });
		...
		it('should handle add node error', async () => {
			...
			const result = await tools.add_node_to_canvas({ path: 'test.canvas', node: {} as CanvasNode });
	...
	describe('Power Tools', () => {
		it('should execute a command', async () => {
			const tools = getObsidianTools(mockApp);
			const obsidianApp = mockApp as ObsidianApp;
			obsidianApp.commands.executeCommandById = vi.fn().mockReturnValue(true);
		...
		it('should handle command execution failure', async () => {
			const tools = getObsidianTools(mockApp);
			const obsidianApp = mockApp as ObsidianApp;
			obsidianApp.commands.executeCommandById = vi.fn().mockReturnValue(false);
		...
		it('should handle command execution error', async () => {
			const tools = getObsidianTools(mockApp);
			const obsidianApp = mockApp as ObsidianApp;
			obsidianApp.commands.executeCommandById = vi.fn().mockImplementation(() => { throw new Error('exec error'); });
		...
		it('should list commands', async () => {
			const tools = getObsidianTools(mockApp);
			const obsidianApp = mockApp as ObsidianApp;
			obsidianApp.commands.listCommands = vi.fn().mockReturnValue([{ id: 'c1', name: 'N1' }]);
		...
		it('should handle list commands error', async () => {
			const tools = getObsidianTools(mockApp);
			const obsidianApp = mockApp as ObsidianApp;
			(obsidianApp.commands as any).listCommands = null;
			const result = await tools.list_commands();
